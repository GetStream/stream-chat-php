...
> [!NOTE]
> In legacy permission system users can never access users nor channels from other teams. In [Permissions V2](/chat/docs/php/chat_permission_policies/) it is possible to alter this behavior using multi-tenant permissions.
...
$response = $client->upsertUser($user);
```
...
In tables below you will find default permission grants for builtin roles that designed for multi-tenant applications. They are useful for [multi-tenant applications](/chat/docs/php/multi_tenant_chat/) only.
...
$response = $client->queryTeamUsageStats(["month" => "2024-01"]);
...
// With pagination
$response = $client->queryTeamUsageStats(["limit" => 10]);
if (isset($response["next"])) {
    $nextPage = $client->queryTeamUsageStats([
        "limit" => 10,
        "next" => $response["next"]
    ]);
}
