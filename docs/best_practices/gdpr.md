...
GDPR gives EU citizens the right to request access to their information and the right to have access to this information in a portable format. Stream Chat covers this requirement with the [Export User](/chat/docs/php/exporting_channels/) method.

This method can only be used with server-side authentication.

```php
$response = $this->serverClient->exportUser($user["id"]);
```
...
- [Deleting a batch of users](/chat/docs/php/update_users/#deleting-many-users/)

- [Deleting a batch of channels](/chat/docs/php/channel_delete/#deleting-many-channels/)
